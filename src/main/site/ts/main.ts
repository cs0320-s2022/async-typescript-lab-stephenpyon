...
function postAndUpdate(): void {
  ...
  }).then((m: Response) => m.json()).then((json: Matches) => updateSuggestions(json.matches)).catch((e) => console.log(e));
...
function updateSuggestions(matches: string[]): void {
  ...
  // @ts-ignore
  matches.forEach((match: string) => suggestions.innerHTML += '<li tabindex="0">' + match + '</li>');
}
...
document.addEventListener('keyup', event => {
  if(event.code == 'KeyS') {
    updateValues('Cancer', 'Scorpio', 'Aquarius').then(() => postAndUpdate())
  }
})
